...
class InfoEngineTest(unittest.TestCase):
    ...
    def test_all(self):
        ...
        self.__test_case_12(info_engine)
        self.__test_case_13(info_engine)

    def __test_case_1(self, info_engine):

        output = info_engine.extract('what is the net weight?', question_type=constants.SPEC,
    ...
    def __test_case_8(self, info_engine):
        ...
        while True:
            ...
            output = info_engine.extract(ques, question_type=question_type,
                                         product=product, pipeline=constants.PIPELINE_1, top_k=3)
    ...
    def __test_case_10(self, info_engine):
        ...
        print('class classifier', output)

        output = classifier_engine.get_classifier_output("What does IRIS mean in hospitals?")
        output = json.loads(output)["response_data"]["Class"]
        print('class classifier', output)

        output = classifier_engine.get_classifier_output("Is India ever qualified for fifa world cup")
        output = json.loads(output)["response_data"]["Class"]
        print('class classifier', output)
    ...
    def __test_case_12(self, info_engine):
        from components.text_similarity.similarity_factory import SimilarityFactory
        from components.engine import constants
        siam_bert = SimilarityFactory.get_text_sim_model(SimilarityFactory.SIAMESE_BERT_MODEL, None)
        import pandas as pd
        import numpy as np
        data = pd.read_excel(constants.SPEC_TEST_DATA, engine='openpyxl', sheet_name='sheet1')
        import time
        question_list =  data['User Question']
        # question_list = question_list[0:256]
        question_list = np.tile(question_list, [5])
        print(len(question_list))
        start = time.time()
        # siam_bert.compute_embeddings_single(question_list, num_workers=0)
        end = time.time()
        print('with num_workers 0', end-start)

        start = time.time()
        siam_bert.compute_embeddings_single(question_list)
        end = time.time()
        print('with num_workers 4', end - start)

    def __test_case_13(self, info_engine):
        srl_cons = info_engine.extract_srl_cons('Why is my washer gives rattling noise when it washes clothes')
        print('srl_cons', srl_cons)
