...
import image_handling_interface as im_interface
...
class Operation(object):
    ...
    def make_triplets(self, operation_section_dictionary):
        ...
        try:
            ...
            for each_section in operation_section_dictionary[cs.COMMON_INFO_KEY][cs.DATA_KEY]:
                ...
                sections_dictionary = each_section[cs.OPERATION]
    ...
    def _create_operation_triplets(self, operation_domain, operation_relation, operation_range,
        ...
        # Check if some figure tags are there inside the desc
        image_content_under_desc = self._get_image_content_under_desc(operation_range_info)
        if image_content_under_desc is not None:
            image_triplets = self._create_triplets_for_image(image_domain=operation_range,
                                                             image_content=image_content_under_desc,
    ...
    def _get_description_for_sections(self, section_info):
        ...
        if cs.DESCRIPTION in section_info:
            temp_desc_full = ". ".join([x.strip(".") for x in section_info[cs.DESCRIPTION]])
            desc_temp = temp_desc_full

        if cs.DESCRIPTION_POINTS in section_info:
            for each_desc_points in section_info[cs.DESCRIPTION_POINTS]:
                if desc_temp is None:
                    desc_temp = ". ".join([x.strip(".") for x in each_desc_points[cs.DESCRIPTION]])
                else:
                    desc_temp += " " + ". ".join([x.strip(".") for x in each_desc_points[cs.DESCRIPTION]])
