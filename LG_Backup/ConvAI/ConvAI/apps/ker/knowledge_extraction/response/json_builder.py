...
from .json_widget_utils import WidgetCards
...
class WidgetConstants(object):
    ...
    PROD_REG_MSG = "Please select any product listed below OR type the product name"
    MODEL_REG_MSG = "Please select any model listed below OR type the model number"
    REG_COMPLETION = "Thanks for the product registration"
    MESSAGE_HEADER_MSG = "Below is the response: "
    PROD_REG_FAIL = "Product Register Fails"
    SUCCESS = "success"
    INVALID_PRODUCT = "Product is not valid"
    INVALID_ALIAS = "Product Alias is not valid"
    ...
    MEDIA_DB = "http://" + ip_address + ":" + port_number + "/" + image_path + "/"
    # MEDIA_DB = "https://www.refrigerator.com/"
    # list of product register messages
    REGISTER_MSGS = [INIT_PROD_REG, "register product", "product register", "registration", "new product"]
...
class JsonBuilder(object):
    ...
    def __init__(self):
        if JsonBuilder.__instance is not None:
            ...
            "refrigerator": {"Refrigerator":["kitchen refrigerator","dinning refrigerator","hall refrigerator"]},
            "air conditioner": {"Air Conditioner":["kitchen air conditioner","dinning air conditioner","hall air conditioner"]},
            "vacuum cleaner": {"Vacuum Cleaner":["vacuum cleaner"]},
            "microwave oven": {"Microwave Oven":["kitchen microwave oven","dinning microwave oven","hall microwave oven"]},
            "dish washer":{"Dish Washer":["dish washer"]}
            }"""

            # convert string to json string
            self.alias_for_products = json.loads(self.product_alias)
    ...
    def load_json_cards(self):
        ...
        """
        logger.debug("load json cards for RCS")
    ...
    def __create_reply_card(self, template, display_text, postback_data):
        ...
        logger.info("replycard=%s", reply_card)
    ...
    def send_prod_types_for_registration(self, model_dict=None, context_not_valid=False):
        ...
        if context_not_valid:
            ...
            for each_prod in retrieved_prod_list:
                logger.info("each_prod=%s", each_prod)
                product_short_name = list(self.alias_for_products[each_prod].keys())[0]
                postback_data = WidgetConstants.PROD_REG_KEY + each_prod
                each_prod_dict = self.__create_reply_card(WidgetCards.carousel_template, product_short_name,
                                                          postback_data)
                logger.info("product each item=%s", each_prod_dict)
            ...
            content = json.dumps(title_content) + self._carousel_identifier + json.dumps(footer_content)
            return content
    ...
    def send_prod_registration_status(self, product_alias, model, product_name=None):
        ...
        content[WidgetConstants.MESSAGE][WidgetConstants.GENERAL_PURP_CARD][WidgetConstants.CONTENT][
            WidgetConstants.DESC] = "Product : " + product_name.capitalize() + "\n" + "Model : " + model + "\n" + "Alias : " + product_alias.capitalize()
    ...
    def __form_bot_response_for_troubleshooting(self, info_mapping, db_resp):
        ...
        # media content is not there and contains only text
            ...
            trob_each_item[WidgetConstants.TITLE] = cs.PROBLEM + ": " + info_mapping[cs.PROP_VALUE]
            trob_each_item[WidgetConstants.DESC] = cs.CAUSE.capitalize() + ": " + resp_cause
