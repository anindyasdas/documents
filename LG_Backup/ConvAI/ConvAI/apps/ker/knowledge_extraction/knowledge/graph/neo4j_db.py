...
logger.debug("=-------CONF pTAH=%s", CONFIG_PATH)
...
class Neo4jDB(object):
    ...
    def __check_population_stats(self, stats_data):
        """
    ...
    def __get_population_resp(self, triplets):
        """
        ...
        logger.log(LG_LOGGING_MODULE_OUTPUT_LVL, NODES_CREATED)
        logger.log(LG_LOGGING_MODULE_OUTPUT_LVL, REL_CREATED)

        # check status of population
        if (len_triplets * TRIPLET) == (NODES_CREATED + REL_CREATED):
            logger.log(LG_LOGGING_MODULE_OUTPUT_LVL, "Population count match")
