...
from .query.query_constants import SpecQuery, TrobQuery, OperationQuery
from .query.query_utils import QueryEngine
import requests

# constant to log the executed query in console using lg_logging module
...
class DBInterface(object):
    ...
    def retrieve_product_type(self, modelno):
        """
            ...
            Returns:
                product_type : str
        """
        product_type, resp_code = self.db_engine.get_product_type(modelno)
        logger.debug("Product type is=%s", product_type)
        return product_type, resp_code

    def retrieve_model_nos(self, product_type):
        """
            ...
            Returns:
                models : list
        """
        models, resp_code = self.db_engine.get_models(product_type)
        logger.debug("Product type is=%s" % str(models))
        return models, resp_code

    def get_problem_key_from_fulltext(self, modelno, question, verb_phrases, noun_phrases):
        """
            ...
            Returns:
                ...
                status code : int
        """
        return self.db_engine.get_problem_key_from_fulltext(modelno, question, verb_phrases, noun_phrases)

    def retrieve_knowledge(self, relation, topic, entity=None,
                           queryintent=cs.CAUSES_SOL_KEY,
                           knowledge_dict=None, entitytype=cs.MODEL):
        """
            retrieves knowledge from graph database by calling Neo4jInterface
            Args:
	            relation - defined relation of template question
                entity - reference entity of which knowledge is to be retrieved
            ...
            Returns:
                answer : str
        """
        answer, resp_code = self.db_engine.retrieve_knowledge(relation, entity, topic,
    ...
    def retrieve_para_from_graph(self, relation, entity, section, entitytype=cs.MODEL):
        """
           ...
           Returns:
               para : str
        """
        logger.debug("relation=%s, entity=%s, section=%s, entitytype=%s", relation, entity, section, entitytype)
        para, resp_code = self.db_engine.retrieve_para_from_graph(relation, entity, section, entitytype)
...
class Neo4jInterface(object):
    ...
    def __prepare_fulltext_index(self):
        """
        ...
        try:
            ...
            logger.error("Exception in dropping=%s", str(e))
        ...
        try:
            ...
            logger.error("Exception in creating=%s", str(e))
    ...
    def __form_range_insert_query(self, node_name, node_type, prop_dict):# pragma: no cover
        ...
        if (prop_dict == None or str(not prop_dict) == "True"):
            query = """MERGE (b:%s{Name:"%s"})""" % (node_type, node_name.replace("\"","\\\""))
        else:
            if "\"" in node_name:
                if cs.ENTITY in str(prop_dict):
                    mod_prop = ",".join("%s:%s" % (key, value.replace("\"","\\\"")) for key, value in
                                        prop_dict.items())
                else:
                    mod_prop = ",".join("%s:\'%s\'" % (key, value.replace("\"","\\\"")) for key, value
                                        in prop_dict.items())
    ...
    def __form_domain_insert_query(self, node_name, node_type, prop_dict):# pragma: no cover
        ...
        """
        if (prop_dict == None or str(not prop_dict) == "True"):
            query = """MERGE (a:%s{Name:"%s"})""" % (node_type, node_name.replace("\"","\\\""))
        else:
            if "\"" in node_name:
                if cs.ENTITY in str(prop_dict):
                    mod_prop = ",".join("%s:%s" % (key, value.replace("\"","\\\"")) for key, value in
    ...
    def __form_query_for_desc(self, entitytype, entity, intent, common_key, specific_problem, prod_type):
        """
    ...
    def __form_query_for_extra_info(self, product_info, query_method=None):
        ...
        extra_info_sub_query = OperationQuery.EXTRA_INFO_QUERY.format(
            ...
            extra_info=extra_info, entity_type=prod_type)
    ...
    def __form_query_for_direct_info(self, product_info, query_method=None):
        ...
        # 2) Check whether to retrieve Procedures and add Procedure Optional Query
        if query_intent in [cs.ProblemTypes.USAGE, cs.ProblemTypes.ATTACH, cs.ProblemTypes.DETACH,
                            cs.ProblemTypes.DESCRIPTION] \
                and intent in [cs.HAS_OPERATION_SECTION, cs.HAS_COMPONENT]:
            procedure_sub_query = OperationQuery.PROCEDURE_QUERY.format(
                part_number_query=OperationQuery.MATCH_PART_NUMBER,
                operation_or_subsection=operation_or_subsection,entity_type=prod_type)
            query = " ".join([query, OperationQuery.UNION_STRING, procedure_sub_query])

        # 3) Check whether to retrieve Features and add Feature Optional Query
        elif intent == cs.HAS_CONTROL_PANEL_FEATURE:
    ...
    def __get_fulltext_cond_query(self, verb_phrases, noun_phrases):
        """
    ...
    def get_problem_key_from_fulltext(self, modelno, question, verb_phrases, noun_phrases):
        ...
        if resp_code == cs.ResponseCode.KER_INTERNAL_SUCCESS:
            ...
            if length <= 0:
                ...
                length = len(results[0]['op_sections'])
                logger.info("Database results len=(%d)", length)
    ...
    def __retrieve_query_for_spec(self, relation, entitytype, entity):
        """
        ...
        logger.debug("after popping relation=%s" % str(intent))

        # 1) Relationships like HAS_POWER_CONSUMPTION, HAS_BATTERY_RUNTIME, HAS_GAS_REQUIREMENTS have properties in
        # relations Needs to be handled separately
        keys_to_exclude = [cs.RANGE, cs.PRODUCT_TYPE_KEY]
        properties_for_relation = ",".join("%s:'%s'" % (key, value) for key, value in
                                           prop_dict.items() if len(value.strip()) > 0 and key not in
                                           keys_to_exclude)
        relations_with_properties = ["HAS_POWER_CONSUMPTION", "HAS_BATTERY_RUNTIME", "HAS_GAS_REQUIREMENTS"]
        if intent in relations_with_properties and len(properties_for_relation.strip()) > 0:
            ...
            query = spec_specific_query.format(model_number=entity, specification_type=intent,
                                               specification_property=properties_for_relation)
        # 2) Else default handling case
        else:
    ...
    def __retrieve_query_for_trob(self, relation, entitytype, entity, queryintent):
        ...
        if relation[cs.INTENT] == cs.HAS_QUESTION:
            # FAQ retrieval
            faq_query = " ".join([TrobQuery.MATCH_MODEL_PART_NUMBERS, TrobQuery.MATCH_FAQ_NODE])
            match_query = faq_query.format(model_number=entity, faq_question=common_key, entity_type=prod_type)
            querykey = cs.SOLUTION_KEY
            match_query = " ".join([match_query, TrobQuery.RETURN_SOLUTION])
        elif intent == cs.DIAGNOSE_WITH_BEEP:
            diagnose_with_beep_query = " ".join([TrobQuery.MATCH_MODEL_PART_NUMBERS, TrobQuery.RETRIEVE_DIAG_BEEP_PROB])
            match_query = diagnose_with_beep_query.format(model_number=entity, entity_type=prod_type)
        elif intent == cs.DIAGNOSE_WITH_LG_THINQ:
            diagnose_with_thinq_query = " ".join([TrobQuery.MATCH_MODEL_PART_NUMBERS, TrobQuery.RETRIEVE_DIAG_THINQ])
            match_query = diagnose_with_thinq_query.format(model_number=entity, entity_type=prod_type)
        else:
            # check if specific solution/all solutions for problem
            if common_key.lower() != specific_problem.lower():
                specific_cause_query = " ".join([TrobQuery.MATCH_MODEL_PART_NUMBERS, TrobQuery.MATCH_SPECIFIC_CAUSE])
                match_query = specific_cause_query.format(model_number=entity, problem_type=intent, property_key=rel_prop_key,
                                                          property_value=common_key,
                                                          specific_cause=specific_problem, entity_type=prod_type)
                #intent, cs.HAS_SOLUTION, prod_type, prod_type,rel_prop_key, common_key, specific_problem)
            else:
                specific_problem_query = " ".join([TrobQuery.MATCH_MODEL_PART_NUMBERS, TrobQuery.MATCH_SPECIFIC_PROBLEM])
                match_query = specific_problem_query.format(model_number=entity, problem_type=intent,
                                                            property_key=rel_prop_key,
                                                            property_value=common_key, entity_type=prod_type)
    ...
    def __remove_toomany_condns(self, query):
        """
           forms cypher query to retrieve knowledge from database
           Args:
               query : str
        ...
        logger.info("query_list:(%s)" % (str(query)))
        return query
    ...
    def __get_specific_dimension(self, rel_schema, node_value):
        """
    ...
    def __get_specific_value(self, result, rel_schema):
        """
           parses py2neo node object cypher query results and get specific
           answer
        ...
        logger.info("updated dict from relationship results=(%s)" % str(resp_dict))

        # convert node val results to dict
        final_dict = self.__convert_node_to_dict(node_value)
        logger.info("Converted dict from node results=(%s)" % str(final_dict))
        node_value = final_dict
        logger.info("@Converted dict from node results=(%s)" % str(node_value))
        ...
        if rel_schema[cs.INTENT] == cs.HAS_DIMENSION or rel_schema[cs.INTENT] == cs.HAS_OVEN_DIMENSION:
            ...
            # if spec key is not looking for specific info, return name info of node
                ...
                answer = node_value[cs.NAME]
        logger.info("###__get_specific_value answer=%s" % (answer))
    ...
    def __parse_structured_content_results(self, results, rel_schema):
        """
           parses py2neo object cypher query results and returns the answer
    ...
    def __parse_graph_results(self, topic, results, rel_schema, result_sts):
        ...
        if result_sts != cs.ResponseCode.KER_INTERNAL_SUCCESS:
    ...
    def __retrieve_by_knowledge_dict(self, relation, entity, topic, entitytype, queryintent, knowledge_dict):
        """
        ...
        # execute query
        results, resp_code = self.graph.run_query(query)
        logger.log(LG_LOGGING_MODULE_OUTPUT_LVL, query)
        length = len(results)
        logger.info("**Database results len=(%d)", length)
    ...
    def __form_and_execute_query(self, relation, entity, topic, entitytype, queryintent,
        ...
        try:
            ...
            elif knowledge_dict is not None:
    ...
    def get_product_type(self, modelno, entitytype=cs.MODEL):
        """
            retrieve product type for the given modelno
            Args:
                modelno : str
        ...
        query = QueryConstants.RETRIEVE_PROD_TYPE % (entitytype, modelno, intent)
        # execute query
        results, resp_code = self.graph.run_query(query)
        for result in results:
    ...
    def get_models(self, product_type, entitytype=cs.MODEL):
        """
            get all models name for the given product type
            Args:
                product_type : str
                entitytype : str
            Returns:
        ...
        if (product_type is None) or (len(product_type) <= 0):
            query = QueryConstants.RETRIEVE_ALL_MODELS % (entitytype, intent,
                                                          entitytype.lower(), cs.RS_PRODUCT_TYPE, entitytype.lower(),
                                                          cs.RS_PRODUCT_TYPE, entitytype.lower())
        else:
            query = QueryConstants.RETRIEVE_PROD_MODELS % (entitytype, intent,
                                                           product_type, entitytype.lower(), cs.RS_PRODUCT_TYPE,
                                                           entitytype.lower(), cs.RS_PRODUCT_TYPE, entitytype.lower())
        # execute query
        results, resp_code = self.graph.run_query(query)
        ...
        if resp_code == cs.ResponseCode.KER_INTERNAL_SUCCESS:
            for result in results:
                models[result[cs.RS_PRODUCT_TYPE]] = result[cs.VALUE]
            logger.debug("models from database=(%s)" % str(models))
        return models, resp_code

    def __convert_recordlist_todict(self, record):
        """
    ...
    def __convert_dict_to_para(self, results):
        """
    ...
    def __make_para_from_cypher_results(self, results):
        """
    ...
    def retrieve_para_from_graph(self, relation, entity, topic, entitytype, subtopic=None):
        """
           retrieves knowledge using entity,entity type,relation details
           from database and forms paragraph
