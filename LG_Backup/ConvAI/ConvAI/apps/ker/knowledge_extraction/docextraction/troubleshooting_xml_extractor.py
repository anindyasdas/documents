...
class TroubleshootingXMLExtractor(XMLExtractor):
    ...
    def _standadize_section_topic_title(self, title_txt):
        """
        standardize the section title to standard title

        Args:
            title_txt: title from manual
        Return:
            standardized title text
        """
        section_names = p.ExtractionConstants.SECTION_NAMING_LIST.keys()

        for section_name in section_names:
            if title_txt in p.ExtractionConstants.SECTION_NAMING_LIST[section_name]:
                return section_name
        return title_txt
    ...
    def _extract_data_from_tag(self, ele_appendix):
        ...
        framed_dict[appendix_title_txt][p.PRODUCT_TYPE_KEY] = self.get_product_type()
        ...
        for ele_section in ele_sections:
            ele_title = ele_section.find(p.XMLTags.TITLE_TAG)
            section_title = self._standadize_section_topic_title(ele_title.text)
            ...
            for ele_topic in ele_topics:
                ...
                topic_key = self._standadize_section_topic_title(topic_key)
                logger.debug("topic_key : %s", topic_key)
                if topic_content is not None:
                    if topic_key in section_dict[section_title]:
                        section_dict[section_title][topic_key].extend(topic_content)
                    else:
    ...
    class TroubleShootingProbCauseExtract:
        ...
        def _get_problem_text(self, ele_troublelistentry):
            ...
            return p.ExtractionConstants.map_error_code(self._get_value_frm_para(ele_paras[0])), ""
        ...
        def _get_frm_solution(self, ele_solution):
            ...
            """
            sol_list = []
            ele_itemizedlist = ele_solution.find(p.XMLTags.ITEMIZEDLIST_TAG)
            ele_listitems = ele_itemizedlist.findall(p.XMLTags.LISTITEM_TAG)
            for ele_listitem in ele_listitems:
        ...
        def _get_from_troublelistitem(self, ele_troublelistitem):
            ...
            if ele_reason is not None:
                key_value_pair[p.REASON_KEY] = self._get_from_reason_tag(ele_reason)
            else:
        ...
        def _get_from_troublelistentry(self, ele_troublelistentry):
            ...
            for ele_troublelistitem in ele_troublelistitems:
                reason_solution_list.append(self._get_from_troublelistitem(ele_troublelistitem))
