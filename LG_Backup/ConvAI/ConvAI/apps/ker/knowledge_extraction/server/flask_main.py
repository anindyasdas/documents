...
uploads_dir = os.path.realpath('.') + "/dataset/bulk_test_data/"
...
def get_answer(json_obj, client_type):
    """
...
def main(input_filepath = None, type="html"):
    if type == "file_based":
        test_report(input_filepath)
    else:
        ip = '0.0.0.0'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file_based_testing",
                        action="store_true",
                        help="Mention this argument if you want to do file based testing of the system")
    parser.add_argument("--input_filepath", "-i",
                        type=str,
                        default="test.csv",
                        help="Specify input file path. with csv filename")
    p_args = parser.parse_args()

    if p_args.file_based_testing:
        if not os.path.exists(p_args.input_filepath):
            print('Input file does not exist or is incorrect')
            sys.exit(0)
        main(p_args.input_filepath)
    else:
        main()
