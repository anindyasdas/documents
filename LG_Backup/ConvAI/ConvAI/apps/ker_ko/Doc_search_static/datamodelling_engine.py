...
class DataModelEngine:
    ...
    def __init__(self, filename):
        ...
        self.section = ""
        self.part_model_mapper_file_path="./partno_modelno_mapper.json"
        self.part_to_model={}
    ...
    def process_sections(self):
        ...
        d = self.update_modelno_partno(d)
    ...
    def update_modelno_partno(self, dict_item):
        """
        This function updates the model no. and part no. in the processed json file
        ...
        new_dict = {cs.XMLTags.BUYERMODEL_TAG: self.model_nums,
                    cs.XMLTags.PARTNUMBER_TAG: self.partnumber}
    ...
    def update_partno_modelno_mapping(self):
        if os.path.exists(self.part_model_mapper_file_path):
            with open(self.part_model_mapper_file_path, 'r', encoding='utf-8-sig') as part_model_mapper_file:
                self.part_to_model = json.load(part_model_mapper_file)
        self.part_to_model[self.partnumber]= "/".join(self.model_nums)
        return self.part_to_model, self.part_model_mapper_file_path
...
if __name__ == "__main__":
    ...
    part_to_model, part_model_mapper_file_path= Pjson_obj.update_partno_modelno_mapping()
    hp.save_file(part_to_model, part_model_mapper_file_path, ftype='json', encoding='utf-8-sig')
