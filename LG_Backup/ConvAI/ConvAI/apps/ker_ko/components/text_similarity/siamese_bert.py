...
import scipy
import pandas as pd
from . import constants, base_question_extraction
...
class SiameseBERT(BaseSimilarityModel):
    ...
    def __init__(self, config):
        ...
        if self.config['language'] == 'english':
            self.model = SentenceTransformer(constants.SIAM_BERT_MODEL_PATH)
        else:
            logger.info("Multiligual bert is loading")
            self.model = SentenceTransformer(constants.SIAM_BERT_MULTILINGUAL_MODEL_PATH)
    ...
    def model_evaluate_bulk(self, test_question_list, question_list=None, question_type=constants.SPEC, top_k=1,
        ...
        for i, test_question in enumerate(test_question_list):
            ...
            logger.debug("predicted question=%s",i)
    ...
    def __get_accuracy(self, data, top_k):
        ...
        logger.info('Accuracy: {}'.format(np.mean(accuracy)))
