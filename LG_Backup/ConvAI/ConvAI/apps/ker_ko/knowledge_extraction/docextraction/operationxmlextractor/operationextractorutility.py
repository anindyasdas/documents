...
class OperationExtractorUtility(object):
    """
        ...
        for ele_listitem in ele_listitems:
            ...
            for children in all_children:
                if children.tag == p.XMLTags.PARA_TAG:
                    ...
                    if p.DESCRIPTION_POINTS in list_items:
                        list_items[p.DESCRIPTION_POINTS].extend(self.extract_from_itemizedlist(children))
                    else:
                    ...
                    list_items[p.XMLTags.CAUTION_TAG] = self.extract_from_caution(children)
                elif children.tag == p.XMLTags.WARNING_TAG:
                    list_items[p.XMLTags.WARNING_TAG] = self.extract_from_warning(children)
    ...
    def _get_value_from_callout_listitem(self, tag):
        ...
        for children in all_childern:
            ...
            if children.tag == p.XMLTags.PARA_TAG:
                ...
                explanation.extend(self.extract_from_itemizedlist(children))
            elif children.tag == p.XMLTags.CAUTION_TAG:
        ...
        for ele_varlistentry in ele_varlistentries:
            ...
            for children in ele_listitem.getchildren():
                if children.tag == p.XMLTags.PARA_TAG:
                    ...
                    note = self.extract_from_note(children)
                elif children.tag == p.XMLTags.ITEMIZEDLIST_TAG or children.tag == p.XMLTags.ORDEREDLIST_TAG:
                    list_items.append(self.extract_from_itemizedlist(children))
    ...
    def extract_from_caution(self, tag):
        ...
        for childern in all_childern:
            ...
            if childern.tag == p.XMLTags.FIGURE_TAG:
                caution_list.update(self.extract_from_figure(childern, self.file_path))
        return caution_list
    ...
    def extract_from_note(self, tag):
        ...
        for childern in all_childern:
            ...
            if childern.tag == p.XMLTags.FIGURE_TAG:
                note_details.update(self.extract_from_figure(childern, self.file_path))
    ...
    def extract_from_warning(self, tag):
        ...
        for childern in all_childern:
            ...
            if childern.tag == p.XMLTags.FIGURE_TAG:
                warning_list.update(self.extract_from_figure(childern, self.file_path))
