...
class OperationXMLExtractor(XMLExtractor):
    ...
    def _extract_from_variablelist_listitem(self, tag):
        ...
        for children in all_children:
            if children.tag == p.XMLTags.PARA_TAG:
                ...
                extract_json[p.XMLTags.NOTE_TAG] = self.operationextractorutility.extract_from_note(
                    children)
    ...
    def _extract_from_simplesect(self, tag):
        ...
        for children in all_childern:
            # topic title is in skip section list don't extract
            if topic_title in p.ExtractionConstants.SKIP_SECTION:
                topic_title = None
                continue
    ...
    def _extract_topic_child3(self, children, previous_tag, previous_title, topic_dict, topic_title):
        ...
        if children.tag == p.XMLTags.SIMPLESECT_TAG:
            ...
            # if the section title is none don't add in main section dict
            if title is not None:
                previous_title = title
                if not bool(content) is False:
    ...
    def _get_section(self, section_title):
        ...
        tag_str_list, tag_list = self._find_chapters(title_list)
        logger.debug('OPERATION : (%s)', tag_list)
        key_value_pair_lst = []
        key_lst = []
        for tag_str,tag in zip(tag_str_list,tag_list):
            ...
            if key == p.OPERATION_WASHER:
                entity_dict[p.ENTITY_PRD_TYPE] = p.GenericProductNameMapping.WASHER_SEC_NAME
                key_value_pair[p.OPERATION]= key_value_pair.pop(key)
                key = p.OPERATION
            elif key == p.OPERATION_DRYER:
                entity_dict[p.ENTITY_PRD_TYPE] = p.GenericProductNameMapping.DRYER_SEC_NAME
                key_value_pair[p.OPERATION] = key_value_pair.pop(key)
                key = p.OPERATION
            else:
                entity_dict[p.ENTITY_PRD_TYPE] = self.get_ent_prd_type(section_title, prd_type)
            key_value_pair.update(entity_dict)
            key_value_pair_lst.append(key_value_pair)
            key_lst.append(key)
        if key_value_pair_lst:
            return key_value_pair_lst, key_lst
        else:
            return None, None
    ...
    def get_operation_data(self):
        ...
        if section_data is not None:
            ...
            for section_data_item in section_data:
                logger.debug("section_data : %s",section_data_item)
                framed_dict[p.COMMON_INFO_KEY][p.DATA_KEY].append(post_process.post_process_data(section_data_item))
