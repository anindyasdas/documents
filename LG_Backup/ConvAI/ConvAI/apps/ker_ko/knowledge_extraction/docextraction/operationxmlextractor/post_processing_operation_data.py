...
class PostProcessOprData(object):
    ...
    def post_process_data(self, data):
        ...
        """
        reframed_json = {}
        ent_prd_type = data[p.ENTITY_PRD_TYPE]
        data = data[p.OPERATION]
        for section in self.post_process_section.keys():
            for internal_section in self.post_process_section[section].keys():
                for topic in self.post_process_section[section][internal_section]:
                    if (section in data) and (internal_section in data) and (topic in data[section][internal_section]):
                        reframe_data = self._reframe_section_data(data[section][internal_section][topic], section,
                                                                  topic)
        ...
        reframed_json[p.ENTITY_PRD_TYPE] = ent_prd_type
