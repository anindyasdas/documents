...
class SpecificationXMLExtractor(XMLExtractor):
    ...
    def _extract_from_topic_generic(self, ele_topic):
        ...
        if valid_flag:
            ...
            framed_json[p.PRODUCT_TYPE_KEY] = prd_type
            framed_json[p.SUB_PRD_TYPE_KEY] = sub_prd_type
    ...
    def get_custom_table_key_values_generic(self, table):
        ...
        prd_type, sub_prd_type = self.get_product_type()
        final_key_value_pair[p.PRODUCT_TYPE_KEY] = prd_type
        final_key_value_pair[p.SUB_PRD_TYPE_KEY] = sub_prd_type
        ...
        for row in ele_rows:
            ...
            for ele_entry in row.findall(p.XMLTags.ENTRY_TAG):
                ...
                if len(ele_paras) > 1:
                    for ele_para in ele_paras:
                        if key_flag:
                            model_list.append(self._return_regex_truncated_model_no(self._get_value_frm_para(ele_para)))
                            value_list.append(self._return_regex_truncated_model_no(self._get_value_frm_para(ele_para)))
                        else:
                            value_list.append(self._get_value_frm_para(ele_para))
                else:
                    if key_flag:
                        model_list.append(self._return_regex_truncated_model_no(self._get_value_frm_para(ele_paras[0])))
                        value_list.append(self._return_regex_truncated_model_no(self._get_value_frm_para(ele_paras[0])))
                    else:
