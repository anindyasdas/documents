...
class TroubleShooting(object):
    ...
    def __init__(self):
        ...
        if TroubleShooting.__instance is not None:
            ...
            if self.use_srl and self.use_cons_parser:
                ...
                self.nlp_engine_client = None
            self.relative_file_path_template = "{part_number}/{t_section_as_per_manual}/{image_file_name}"
    ...
    def __refer_schema_create_triplet(self, domain_relation_range,
        ...
        try:
            ...
            logger.debug("domain type=%s rangetype=%s relation=%s" % (domain_type,
                                                                      range_type,
                                                                      relation_name))
            relations_to_add_properties = [cs.HAS_ERROR, cs.HAS_NOISE, cs.HAS_PROBLEM, cs.HAS_COOLING_PROBLEM,
                                           cs.HAS_ICE_PROBLEM, cs.HAS_WIFI_PROBLEM, cs.HAS_TROUBLESHOOTING_PROBLEM,
                                           cs.DIAGNOSE_WITH_LG_THINQ, cs.DIAGNOSE_WITH_BEEP, cs.DIAGNOSE_WITH_WIFI,
                                           cs.KO_DIAGNOSING_FAULT_BEEP, cs.HAS_PROCEDURE ]
            if generickey in relations_to_add_properties:
                ...
                if generickey == cs.HAS_QUESTION:
                    ...
                    # Check for not None
                    # relation_prop_dict = dict()
                    self._fill_relation_dict(issue_type_value=issue_type_value, relation_prop_dict=relation_prop_dict)
                ...
                relation = utils.Relation(relation_name, relation_prop_dict)
                # create  model node
                # If the relation name is "TypeOf" Apply the knowledge dict (properties to the range node)
                if relation_name == cs.REL_TYPE_OF:
                    domain = utils.Node(domain_type, domainvalue, None)
                    # create end node
                    rangenode = utils.Node(range_type, rangevalue, knowledge_dict)
                else:
    ...
    def __make_triplets_causesolution(self, section, problem, cause, solution,
                                      triplets_list):
        """
        Create triplets for Causes and Solution starting from PartNumber
        (PartNumber)-[:HAS_SOME_PROBLEM]->(Cause)
        ...
        for eachcause in cause:
            ...
            # Create First level nodes (PartNumber)-[:HAS_SOME_PROBLEM]-(Cause)
            error_code_with_comma = commonkey is cs.HAS_ERROR and ("," in problem)
            #  Split error codes with comma
            #  Ex: "tE1, tE2, tE4, tE5, tE6" will be split as ['tE1', 'tE2', 'tE4', 'tE5', 'tE6']
            if error_code_with_comma:
                data_for_creating_triplet = commonkey, self.part_no, eachcause, knowledge_dict, problem, section
                comma = r","
                error_code_triplets_list = self._split_error_codes(data_for_creating_triplet, delimiter=comma)
                triplets_list.extend(error_code_triplets_list)
            else:
                domain, relation, rangenode = self.__refer_schema_create_triplet(
                    (self.part_no, commonkey, eachcause), knowledge_dict,
                    problem, issue_type_value=section, ent_prd_type=self.ent_prd_type,
                    partnumber=self.part_no)
                # create each Node_Relation
                triplets_list.append(self._convert_triplet_to_dictionary(domain, rangenode, relation))
        # Create Next level nodes (Cause)-[:HAS_SOLUTION]->(Solution)
    ...
    def _split_error_codes(self, data_for_creating_triplet, delimiter):
        """
        To Split each error code based on comma or a space
        EX: "tE1, tE2, tE4, tE5, tE6" will be split as ['tE1', 'tE2', 'tE4', 'tE5', 'tE6']

        Args:
            data_for_creating_triplet: info for creating the triplet
            delimiter: space or comma

        Returns:
            list of error code triplets
        """
        commonkey, each_model, eachcause, knowledge_dict, problem, section = data_for_creating_triplet
        triplets_list = []
        error_code_list = [x.strip() for x in re.split(delimiter, problem) if x]
        for each_error_code in error_code_list:
            domain, relation, rangenode = self.__refer_schema_create_triplet(
                (each_model, commonkey, eachcause), knowledge_dict,
                each_error_code, issue_type_value=section, ent_prd_type=self.ent_prd_type, partnumber=self.part_no)

            triplets_list.append(self._convert_triplet_to_dictionary(domain, rangenode, relation))
        return triplets_list

    def _convert_triplet_to_dictionary(self, domain, range_node, relation):
        """
        Create a node relation and convert the domain, range_node, relation to dictionary format
        """
        return utils.NodeRelation(domain.__dict__,
                                  relation.__dict__,
                                  range_node.__dict__).__dict__
    ...
    def __make_triplets_for_troubleshooting(self, section, section_info, en_prd_type=None):
        """
        Make triplets for troubleshooting sections based on the Schema, Part Number will be the central node
        Args:
            section: Troubleshooting sub section like Error codes, Operation, Use etc.,
            section_info: section info like list of causes ans solutions
            en_prd_type: sub product type like washer/dryer
        ...
        try:
            triplets_list = []
            logger.info("section=%s, section_info=%s ", str(section), str(section_info))

            # If the section info is FAQ or Other sections call the corresponding functions accordingly
            if section == cs.FAQ:
                triplets_list = self.__make_triplets_forfaq(section_info)
            else:
                ...
                for each_sub_section in section_info:
                    for each_problem, cause_sol_list in each_sub_section.items():
                        ...
                        for each_item in cause_sol:
                            ...
                            self.__make_triplets_causesolution(section, error_code_or_problem,
    ...
    def __make_triplets_foreachfaq(self, modelno, question, solution,
        ...
        try:
            ...
            domain, relation, rangenode = self.__refer_schema_create_triplet(
                (modelno, commonkey, question), issue_type_value=cs.FAQ)
            # create each Node_Relation
            ...
            for eachsolution in solution:
                eachsolution = eachsolution.capitalize()
                domain, relation, rangenode = self.__refer_schema_create_triplet(
                    (question, commonkey, eachsolution))
                # create each Node_Relation
                eachtriplet = utils.NodeRelation(domain.__dict__,
    ...
    def __make_triplets_forfaq(self, faq_data):
        """
        Make triplets for FAQ Section
        """
        try:
            triplets_list = []
            list_value = []
            logger.debug("dict_object=%s", str(faq_data))
            ...
            for key, value in faq_data.items():
                ...
                for eachitem in value:
                    ...
                    logger.debug("question=(%s) answer=(%s)" % (key,
                                                                str(list_value)))
                    self.__make_triplets_foreachfaq(self.part_no, key, list_value,
                                                    triplets_list)
    ...
    def __make_triplet_for_producttype(self, modelno, product_type):
        ...
        # Add sub product type as a property
        sub_product_type_dict = {cs.SUB_PRODUCT_TYPE: self.sub_prd_type}
        # create triplet for product type and model
        domain, relation, rangenode = self.__refer_schema_create_triplet(
            (modelno, prodtype_key, product_type), knowledge_dict=sub_product_type_dict)
    ...
    def __make_triplets_for_diagnosing_a_fault(self, section, section_info):
        """
        Create triplets for the section Diagnosing a fault with PartNumber as Domain node
        Args:
        ...
        try:
            ...
            each_diagnosing_a_fault_triplet = self._refer_schema_create_triplet((self.part_no, commonkey,
                                               section),
                                              range_props=range_prop)
            triplets_list_local.append(each_diagnosing_a_fault_triplet)
            ...
            if cs.XMLTags.PROCEDURE_TAG in section_info:
                ...
                elif (section == cs.DIAGNOSING_FAULT_BEEP) or (section == cs.KO_DIAGNOSING_FAULT_BEEP):
                    TROB_REL = cs.DIAGNOSE_WITH_BEEP
                elif (section == cs.DIAGNOSING_FAULT_WIFI) or (section == cs.KO_DIAGNOSING_FAULT_WIFI):
                    TROB_REL = cs.DIAGNOSE_WITH_WIFI
    ...
    def _create_image_information(self, part_no, image_content):
        """
        Function create the relative path for the images
        ...
        if os.path.isfile(file_path_local):
            ...
            # Get the image name
            first_path, image_file_name = os.path.split(file_path_local)
            t_section_as_per_manual = os.path.basename((first_path))
            # MAKE THE DIRECTORIES IN THE FORM: partnumber\\t_00040046_r2\\smart_diagnosis.png
            relative_file_path = self.relative_file_path_template.format(
                part_number=part_no,
                t_section_as_per_manual=t_section_as_per_manual,
                image_file_name=image_file_name)

            logger.info("relative path " + str(relative_file_path))
            ...
            final_image_content[cs.ExtractionConstants.FILE_PATH] = relative_file_path
    ...
    def _create_triplets_for_image(self, image_domain, image_content, image_domain_props, image_domain_type,
        ...
        # Call the image handling function for getting the relative path
        image_response = self._create_image_information(self.part_no, image_content)
    ...
    def _get_product_type(self, new_dict_object):
        ...
        # Standardize the sub_prd_type
        sub_prd_type =  cs.get_generic_product_name(sub_prd_type)
    ...
    def _make_triplets_internal(self, dict_object):
        ...
        # @Deprecated (All the nodes will be part number centric)
        ...
        for sub_section, sub_section_content in data_list.items():
            for section, section_info in sub_section_content.items():
                if (section == cs.XMLTags.SUMMARY_TAG) or (section == "Water Filter"):
                    continue
                triplets_list_temp = []
                ...
                if (sub_section == cs.BFR_CALL_FOR_SERVICE) or \
                        ...
                        (sub_section == cs.WASH_DRY_CMN) or (sub_section == cs.TROUBLESHOOTING):
                    triplets_list_temp = self.__make_triplets_for_troubleshooting(section, section_info,
                                                                                  self.ent_prd_type)
                elif sub_section == cs.DIAGNOSING_FAULT:
                    triplets_list_temp = self.__make_triplets_for_diagnosing_a_fault(section, section_info)

                triplets_list = triplets_list + triplets_list_temp

        # create triplet for product type and PartNumber
        triplet = self.__make_triplet_for_producttype(self.part_no, product_type)
        triplets_list.append(triplet.__dict__)
        # Triplets with HAS_PART_NUMBER will not be created
        # part_number_triplet = self._make_triplet_for_part_number(self.part_no)
    ...
    def _get_ent_prd_type(self, section, prd_type):
        ...
        if (section in cs.GenericProductNameMapping.INT_SEC) or (((section == cs.DIAGNOSING_FAULT.lower())\
                or (section == cs.KO_DIAGNOSING_FAULT_BEEP)) and self.sub_prd_type == cs.ExtractionConstants.KEPLER_PRD):
