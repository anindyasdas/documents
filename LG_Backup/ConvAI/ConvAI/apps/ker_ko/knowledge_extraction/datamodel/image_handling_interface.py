...
class ImageHandlingInterface(object):
    ...
    def __init__(self):
        self.relative_file_path_template = "{product_type}/{part_no}/{section}/{sub_section}/"
    ...
    def get_image_information(self, product_type, section, sub_section, part_no, image_content):
        ...
        if os.path.isfile(file_path_local):
            logger.debug("Inside image handling interface : Image file exists")
            # Get the image name
            _, image_file_name = os.path.split(file_path_local)

            # MAKE THE DIRECTORIES IN THE FORM: product_type/section/sub_section/part_no
            ...
            try:
                ...
                logger.info(
                    "Inside image handling interface : relative path is available")
            except FileExistsError as exc:
                logger.info(
                    "Inside image handling interface : relative path is not available" + str(exc))
                os.makedirs(os.path.join(IMAGE_FOLDER_PATH, relative_file_path), exist_ok=True)
            ...
            if os.path.isfile(os.path.join(copy_dst, image_file_name)):
                ...
                logger.info(
                    "Inside image handling interface : Image is not available in relative path")
