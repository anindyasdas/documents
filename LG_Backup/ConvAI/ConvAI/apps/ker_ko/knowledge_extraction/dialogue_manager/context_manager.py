...
class ContextManager(object):
    ...
    def get_partno_context(self):
        ...
        if (pref_product is not None) and (len(pref_product.strip()) > 0):
            pref_partno_key = Preference.get_partno_pref_value(pref_product)
            return pref_partno_key
        else:
            return None
    ...
    def update_partno_context(self, lpart_no, prd_type=None):
        """
        check the part no context with preference
        ...
        logger.debug("updating the model context : %s prd_type: %s", lpart_no, prd_type)
        if lpart_no is not None:
            pref_part_no = Preference.get_partno_pref_value(prd_type)
            if lpart_no != pref_part_no:
                Preference.update_partno_pref(lpart_no, prd_type)
                pref_part_no = Preference.get_partno_pref_value(prd_type)
                return pref_part_no
            else:
                return pref_part_no
    ...
    def get_product_context(self):
        ...
        if (pref_product is not None) and (len(pref_product.strip()) > 0):
            # Reference prd = Preference.get_product_pref_value(pref_product)
            return pref_product
        else:
            return None
    ...
    def update_product_context(self, main_product, sub_prd):
        ...
        if main_product is not None:
            ...
            if sub_prd is not None:
                # for ref if lsub_prd != sub_prd:
                #     Preference.update_product_pref(sub_prd, main_product)
                Preference.update_product_pref(main_product, main_product)
                logger.debug("pref_prd :%s %s", main_product, sub_prd)
                return main_product, main_product
            else:
                pref_product = Preference.get_pre_prd()
