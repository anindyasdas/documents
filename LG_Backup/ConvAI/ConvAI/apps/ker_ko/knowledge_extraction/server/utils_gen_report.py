...
def testandupdate_report(report_path, obj, input_json, client_type):
    ...
    try:
        ...
        if ext == SUPPORTED_FORMAT[0]:
            data_df = pandas.read_csv(report_path, header=[0], encoding="utf-8-sig")
    ...
    try:
        ...
        if ext == SUPPORTED_FORMAT[0]:
            data_df.to_csv(report_path, index=False, encoding="utf-8-sig")
        elif ext == SUPPORTED_FORMAT[1]:
            data_df.to_excel(report_path, index=False, encoding = "utf-8-sig" )
        return
    except Exception as e:
        logger.error("Error:Updating report in csv" + str(e))
        return
