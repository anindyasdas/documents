...
class EntityDetection(nn.Module):
    ...
    def forward(self, text):
        ...
        if self.config.entity_detection_mode.upper() == 'LSTM':
            ...
            exit(1)
        #print("outputs:", outputs.shape) (sequence length, batch_size,hidden_size*2 bidirectional) 
        tags = self.hidden2tag(outputs.view(-1, outputs.size(2))) #sequence length*batch_size, hidden_size*2 bidirectional
