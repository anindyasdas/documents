...
EMB_BATCH_SIZE=100
emb_file_name='emb_ko_wm_MFL71485465_.json'
PART_NO="MFL71485465"
...
class DocBasedQaEngine(object):
    def __init__(self, embedding_list):
        ...
        self.para_item=  embedding_list[5]
        self.manual_content_dict = {}
        self.json_filename = os.path.join("Manual_json",
                                          os.path.basename(embedding_list[6]).split("\\")[-1])
    ...
    def load_passages(self):
        passages = []
        for item in self.para_item:
            value= item[0]
            head= item[1]
            sub_section_heading=split_value_key_chain(value)[-1]
            value_obj = eval(self.jsonfile_str + value)
            head_section=[]
            if type(value_obj) == list and len(value_obj)>=1:
                if head!='':
                    head_section=[head + " " + "섹션. "]
                value_obj = head_section + util.get_list(value_obj)
                passage = " ".join(value_obj)
                passages.append(passage)
            # if value is empty list and subsection title is a long sentence (len 25), we consider the subsection as paragraph
            elif type(value_obj) == list and len(value_obj)==0 and len(sub_section_heading) >25:
                
                value_obj = [sub_section_heading]
                passage = " ".join(value_obj)
                passages.append(passage)
        return passages
...
#target="섬유 유연제가 넘치지 않게 MAX(기준선)a 이하까지 넣으세요. 기준선을 넘을 경우 섬유 유연제가 드럼 안으로 바로 투입될 수 있습니다"
    #for num, idc in enumerate(key_indices):
    #    if target in keys[idc]:
    #        print(num)
    

    p=retrieve_passages(key_indices, inverted_dict, corpus)
    for num, pa in enumerate(p):
        if target in pa:
            print("rank:", num)
