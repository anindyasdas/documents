...
class InteractionModule:
    ...
    def answer_question(self, question):
        ...
        for opt_no, idc in enumerate(key_idc[:MAX_OPT]):
            ...
            new_str = process_str_(self.values[idc], self.norm_keys[idc])
            ...
            self.keys_score.append(res[idc])
            self.values_option.append(self.values[idc])
        del res
    ...
    def answer_question1(self, question):
        ########Compute Question Embedding#################
        tok_list=[]
        candidate_embeddings=[]
        for token in tokenize(question.lower()):
    ...
    def paraqa1(self, op_feed, questions):
        """
    ...
    def string_matching1(self, op_feed, ques_processed, key_idc):
        if op_feed==2:
            score=[]    
            for  idc in key_idc:
    ...
    def string_matching(self):
        ...
        if op_feed==2:
            score=[]    
            for  idc in self.key_idc:
                #score.append(fuzz.token_sort_ratio(ques_processed,keys[idc]))
                score.append(fuzz.token_set_ratio(self.ques_processed,self.keys[idc]))
            score= np.asarray(score)
            idc_str=(-score).argsort()
            #sys.stdout.write("Does the following matches your description? \n")
            ##############################
            self.values_option_1=[]
            self.keys_option_1=[]
            self.keys_score_1=[]
            #sys.stdout.write("MATCHES:\n")
            for opt_no, idc in enumerate(idc_str[:MAX_OPT]):
                key_index= self.key_idc[idc]
                #self.keys_option_1.append(self.norm_keys[key_index])
                new_str = process_str_(self.values[key_index], self.norm_keys[key_index])
                self.keys_option_1.append(new_str)
                #sys.stdout.write(str(opt_no+1) +". "+ self.norm_keys[idc]+"\n")
                #self.values_option.append(self.jsonfile_str+self.values[idc])
                self.values_option_1.append(self.values[key_index])
                self.keys_score_1.append(score[idc])
            self.keys_option_1.append("None of the above")
