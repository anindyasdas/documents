...
def sentence_level_IE(new_sentences, triple):
...
class SpacyNER:
    ...
    def ner_to_dict(self,ner, NER_dict):
        """
        Expects ner of the form list of tuples 
        """
        ner_dict = NER_dict
        for tup in ner:
            if tup[0] not in ner_dict:
...
file= open('SAFETY_step1.txt', 'w', encoding='utf8')
NER_dict={}
for item in triple:
    if item[0] not in NER_dict:
        NER_dict[item[0]]='OTHER'
...
spacy_ner = SpacyNER()
named_entities = spacy_ner.ner(text)
named_entities = spacy_ner.ner_to_dict(named_entities, NER_dict)
#print(named_entities)


text=text.strip()
sentences= split_sentences(text)
triple, new_sentences =extract_clausal_rel(sentences, triple)

file= open('SAFETY_step2.txt', 'w', encoding='utf8')
for item in triple:
    print(item[0],'\t',item[1],'\t',item[2],file=file)      
file.close() 


triple= sentence_level_IE(new_sentences, triple)

file= open('SAFETY_step3.txt', 'w', encoding='utf8')
for item in triple:
    print(item[0],'\t',item[1],'\t',item[2],file=file)      
file.close() 
