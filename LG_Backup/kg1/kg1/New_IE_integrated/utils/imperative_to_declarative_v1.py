...
Works for Imperative/Declarative but not for 
...
class ImperativeToDeclarative:
    """
    class is used to output the declarative sentence for a given imperative sentences
    """
    def __init__(self):
    ...
    def process_subtree(self, subtrees): 
        ...
        if check_do.lower() == 'do' and check_not.lower() == 'not' and len(new_subtrees) ==1: 
            ...
            trip, new_sent = self.create_new_sent(vb, np, pp, "should not")
        else:
            vb, np, pp = self.process_positive(subtrees)
            vb, np, pp = process_string(vb), process_string(np), process_string(pp)
            trip, new_sent = self.create_new_sent(vb, np, pp, "should")
...
if __name__=="__main__":
    imp_to_dec = ImperativeToDeclarative()
    while True:    
        sent=input('>')
