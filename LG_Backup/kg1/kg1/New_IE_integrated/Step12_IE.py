...
def mask_hyphen(text):
    text = re.sub('\s*-\s*', 'hph', text)
    return text
def decode_hyphen(text):
    text = re.sub('hph', '-', text)
    return text
            ...
            for item in extractions:
                ...
                if context == None:
                    for arg2_item in arg2s:
                        processed_arg2= process_string(arg2_item['text'].strip())
                        #print('arg2:', arg2_item['text'])
                        rel_rev = rel + " reverse" 
                        t.add((arg1, rel, processed_arg2))
                        t.add((processed_arg2, rel_rev, arg1))
...
def concept_relations(triple):
    ...
    for idx in range(len(triple)):
        ...
        if type(item[2])==list:
            ...
            named_item=item2
            #property_dict[named_item] = item2
            ...
            print('simple sentences:', simple_sentences)
    ...
    for item in triple:
        ...
        item2_flag= True
        if type(item[0]) !=list:
            item0=decode_hyphen(item[0])
            if item0.lower() in stop_words:
                item0_flag = False
        else:
            item0=item[0]
            
        item1="_".join(decode_hyphen(item[1]).split())
        if type(item[2]) != list:
            item2= decode_hyphen(item[2])
            if item2.lower() in stop_words:
                item2_flag= False
        else:
            item2= item[2]
            
        item=(item0, item1, item2 )
        if item not in new_triple and item0_flag and item2_flag:
...
if __name__=="__main__":
    ...
    file= open(outname, 'w')
    if model_nums: #if not None 
        print(model_nums,file=file)
