...
class TransEParam(nn.Module):
    def __init__(self, args, embed_matrix,rel2words):
        super(TransEParam, self).__init__()
        self.args = args
        ...
        print("self.args.num_nodes:", self.args.num_nodes)
    ...
    def _calc(self, h, t, r):
        ...
        ret = torch.norm(h + r - t, self.args.p_norm, -1)
        #print("ret:", ret.shape)
        return ret
    ...
    def get_loss(self,samples,edges,node_id):
        ...
        score = self._calc(sub_embed, obj_embed, rel_embed)
        predict_loss = self.loss(score)
        
        return predict_loss
    
    def save_embeddings(self, data):
        fi = open('graph.pkl', 'wb')
        graph_embeddings = {}
        rel = np.array(list(data.id2rel.keys()))
        num_nodes = len(data.ent2id)
        ents = torch.arange(0, num_nodes, dtype=torch.long)
        edges = torch.tensor(data.edges,dtype=torch.long)
        if args.use_cuda: 
            ents = ents.cuda()
            edges = edges.cuda()
        r_embed,ent_embed = self.get_embed(edges,ents,rel)
        graph_embeddings['entity_emb'] = ent_embed
        graph_embeddings['relation_emb'] = r_embed
        pkl.dump(graph_embeddings, fi)
        fi.close()
...
if __name__ == '__main__':
    ...
    ### Mode
    parser.add_argument('-mode',       dest='mode',       default='train', choices=['train', 'eval'],           help='Mode of execution')
