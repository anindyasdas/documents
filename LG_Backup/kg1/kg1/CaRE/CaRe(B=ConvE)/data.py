...
class load_data():
    ...
    def get_phrases(self,file_path):
        f = open(file_path,"r", encoding='utf8').readlines()
        phrase2id = {}
        id2phrase = {}
        word2id = set()
        for line in f[1:]:
    ...
    def get_phrase2word(self,phrase2id,word2id):
        ...
        for phrase in phrase2id:
            phrase_ = re.sub('\[', '', phrase)
            phrase_ = re.sub('\]', '', phrase_)
            phrase_ = re.sub('\.', ' ', phrase_).strip()
            phrase_ = re.sub('\s{2,}', ' ', phrase_)
            phrase_ = re.sub('\s*-\s*', '-', phrase_)
            phrase_ = re.sub(' ', '_', phrase_)
            words = []
            for word in phrase_.split('_'): words.append(word2id[word])
