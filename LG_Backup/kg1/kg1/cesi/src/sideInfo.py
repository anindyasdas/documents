...
class SideInfo(object):
    ...
    def process(self):
        ...
        for triple in self.triples:            # Get all subject, objects and relations
            ...
            self.entnm2ac[triple['triple'][0]] = self.entnm2ac.get(triple['triple'][0], set())
            self.entnm2ac[triple['triple'][0]].add(triple['triple_ac'][0])
            self.relnm2ac[triple['triple'][1]] = self.relnm2ac.get(triple['triple'][1], set())
            self.relnm2ac[triple['triple'][1]].add(triple['triple_ac'][1])
            self.entnm2ac[triple['triple'][2]] = self.entnm2ac.get(triple['triple'][2], set())
            self.entnm2ac[triple['triple'][2]].add(triple['triple_ac'][2])
            
            self.entac2nm[triple['triple_ac'][0]] = triple['triple'][0]
            self.relac2nm[triple['triple_ac'][1]] = triple['triple'][1]
            self.entac2nm[triple['triple_ac'][2]] = triple['triple'][2]
    ...
    def wordnetLinking(self):
        ...
        for trp in self.triples:
            ...
            for sentence in trp['src_sentences']:
                ...
                words_sub = word_tokenize(raw_sub)
                words_obj = word_tokenize(raw_obj)
                words_rel = word_tokenize(raw_rel)

                ''' not None is returned when lesk is successful '''
                for word in words_sub:
                    if word not in self.stopwords:
                        self.ent2wnet[sub_id] = self.ent2wnet.get(sub_id, set())
                        res = lesk(sent, word) 
                        if res: self.ent2wnet[sub_id].add(res.name())
                for word in words_obj:
                    if word not in self.stopwords:
                        self.ent2wnet[obj_id] = self.ent2wnet.get(obj_id, set())
                        res = lesk(sent, word) 
                        if res: self.ent2wnet[obj_id].add(res.name())
                for word in words_rel:
                    if word not in self.stopwords:
                        self.rel2wnet[rel_id] = self.rel2wnet.get(rel_id, set())
                        res = lesk(sent, word) 
                        if res: self.rel2wnet[rel_id].add(res.name())
    ...
    def initVariables(self):
        ...
        self.entnm2ac = {}        # entity normalized to actual entity
        self.relnm2ac = {}        # Relationnormalized to actual relation
        
        self.entac2nm = {}        #Actual entity to normalized entity
        self.relac2nm = {}        #Actual relation to normalized relation
