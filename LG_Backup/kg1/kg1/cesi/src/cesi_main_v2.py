...
class CESI_Main(object):
    ...
    def cluster(self):
        ...
        if not checkFile(fname1) or not checkFile(fname2):
            ...
            dumpClusterAc(fname1, self.ent_clust, self.side_info.id2ent, self.side_info.entnm2ac)
            dumpClusterAc(fname2, self.rel_clust, self.side_info.id2rel, self.side_info.relnm2ac)
        else:
