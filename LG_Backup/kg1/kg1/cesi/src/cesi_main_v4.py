...
#python ppdb/ppdb_server.py -port 9997
#python src/cesi_main_v4.py -data manual -input data_out -name out_new
...
class CESI_Main(object):
        ...
        if not checkFile(fname):
            ...
            with codecs.open(args.data_path, encoding='utf-8', errors='ignore') as f:
                ...
                for line in f:
                    ...
                    sub, rel, obj = proc_ent_n(sub), proc_rel(rel), proc_ent_n(obj)        # Get Morphologically normalized subject, relation, object
                    if len(sub) == 0  or len(rel) == 0 or len(obj) == 0: continue              # Ignore incomplete triples

                    trp['triple']         = [sub, rel, obj]
                    trp["triple_ac"]      = [sub_1, rel_1, obj_1]
    ...
    def cluster(self):
        ...
        fname3 = self.p.out_path + '/cesi_npclust_ent.txt'
        fname4 = self.p.out_path + '/cesi_npclust_rel.txt'
        fname5 = self.p.out_path + '/ent2id.txt'
        fname6 = self.p.out_path + '/rel2id.txt'
        fname7 = self.p.out_path + '/trip.txt'
        ...
        if not checkFile(fname1) or not checkFile(fname2):
            ...
            #dumpClusterAc(fname1, fname3, self.ent_clust, self.side_info.id2ent, self.side_info.entnm2ac)
            #dumpClusterAc(fname2, fname4, self.rel_clust, self.side_info.id2rel, self.side_info.relnm2ac)
            #printitem2id(fname5, self.side_info.ent2id, self.side_info.entnm2ac)
            #printitem2id(fname6, self.side_info.rel2id, self.side_info.relnm2ac)
            dumpCluster_n(fname1, fname3, self.ent_clust, self.side_info.id2ent)
            dumpCluster_n(fname2, fname4, self.rel_clust, self.side_info.id2rel)
            printitem2id_n(fname5, self.side_info.ent2id)
            printitem2id_n(fname6, self.side_info.rel2id)
            with open(fname7, 'w') as f7:
                f7.write(str(len(self.side_info.triples)) + '\n')
                for trip in self.side_info.trpIds:
                    f7.write(str(trip[0])+'\t'+ str(trip[1])+'\t'+ str(trip[2])+ '\n')
        else:
            self.logger.info('\tLoading cached Clustering')
            #self.ent_clust = loadClusterAc(fname1, self.side_info.ent2id, self.side_info.entac2nm)
            #self.rel_clust = loadClusterAc(fname2, self.side_info.rel2id, self.side_info.relac2nm)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-data',         dest='dataset',     default='data',             help='Dataset to run CESI on')
    parser.add_argument('-input',         dest='input_file',         default='data_out',             help='Dataset file name')
    ...
    if args.name == None: args.name = args.dataset + '_' + args.input_file + '_' + time.strftime("%d_%m_%Y") + '_' + time.strftime("%H:%M:%S")
    ...
    args.data_path = args.data_dir + '/' + args.dataset + '/' + args.input_file  # Path to the dataset
